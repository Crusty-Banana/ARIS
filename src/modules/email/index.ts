...
const getVerificationEmailHtml = (name: string, url: string) => `
  <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px;">
    ...
    <p style="font-size: 0.8em; color: #aaa;">Your App Name</p>
  </div>
`;

const getPasswordResetEmailHtml = (name: string, url: string) => `
  <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px;">
...
export async function sendPasswordResetEmail(to: string, name: string, token: string) {
    const resetUrl = `${process.env.NEXT_PUBLIC_APP_URL}/auth/reset-password/${token}`; // Link to your frontend page
