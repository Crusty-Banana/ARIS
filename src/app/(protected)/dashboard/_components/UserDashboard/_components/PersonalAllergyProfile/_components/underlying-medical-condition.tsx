...
export function UnderlyingMedicalConditionsCard({
  conditions,
  onUpdate,
}: UnderlyingMedicalConditionsCardProps) {
        ...
        <div className="space-y-2">
          {conditions && conditions.length > 0 ? (
            conditions.map((condition, index) => (
              <div
                key={index}
                className="flex items-center justify-between bg-white/70 p-2 rounded-md border border-cyan-200"
              >
