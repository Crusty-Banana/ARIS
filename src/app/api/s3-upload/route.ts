...
export async function POST(
  ...
  try {
    ...
    const parsedBody = AddFileToS3$Params.safeParse({ file: buffer, fileName: file.name, contentType: file.type });
    ...
    return NextResponse.json({ result, message: "File uploaded successfully" }, { status: 200 });
  } catch (error) {
