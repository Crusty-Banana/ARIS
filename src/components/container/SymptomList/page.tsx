...
export function SymptomList({ symptoms, onUpdate, onDelete }: SymptomListProps) {
  ...
  const filteredAndSortedSymptoms = symptoms
    .filter((symptom) => symptom.name[localLanguage].toLowerCase().includes(searchTerm.toLowerCase()))
    .sort((a, b) => {
      let comparison = 0
      switch (sortBy) {
        case "name":
  ...
  return (
    ...
    <Card className="bg-gradient-to-br from-cyan-50 to-blue-50 border-cyan-200">
      <CardHeader>
        ...
        <div className="flex gap-2">
          ...
          <Select value={sortBy} onValueChange={(value) => setSortBy(value as SymptomSortOption)}>
    ...
    {(selectedSymptom !== null) && <SymptomDetailModal 
